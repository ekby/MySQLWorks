...
public class CourseDaoImpl implements Dao_idx1<Course> {
    ...
    public ArrayList<Course> SelectByTeacherID(Connection connection, int tid) {
        ...
        ArrayList<Course> arrayList;
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT * FROM course WHERE cteacherid=?", ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);  // 为了下文让指针能移动
